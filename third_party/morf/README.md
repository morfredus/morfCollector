...
Copie autonome de la bibliothèque partagée de morfredus utilisée par morfCollector :

- `beacon/` — **morfBeacon** : supervision LAN (heartbeat UDP « je suis actif » +
  endpoint HTTP `/status`). morfCollector s'en sert pour s'annoncer sur le réseau.

> morfCollector n'embarque **que** morfBeacon (contrairement à ComponentHub, qui
