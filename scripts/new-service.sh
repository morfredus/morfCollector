...
#     morfCollector  -> <NomCamel>   (projet / bibliotheque / cible CMake)
#     morfcollector         -> <nom>        (namespace, dossier include, binaire,
#                                           unite systemd, fichier de config)
#     MORFCOLLECTOR         -> <NOM>        (prefixe de macro)
#
# Usage :
...
grep -rlZ --binary-files=without-match \
     -e 'morfCollector' -e 'morfcollector' -e 'MORFCOLLECTOR' \
     "$DEST" 2>/dev/null | while IFS= read -r -d '' f; do
    case "$f" in */third_party/morf/beacon/*) continue;; esac  # ne pas toucher au code vendore
    sed -i -e "s/morfCollector/$CAMEL/g" \
           -e "s/morfcollector/$LOWER/g" \
           -e "s/MORFCOLLECTOR/$UPPER/g" "$f"
done

# --- 3. Renommer les fichiers/dossiers portant l'ancien nom --------------
mv "$DEST/include/morfcollector"                 "$DEST/include/$LOWER"
mv "$DEST/config/morfcollector.example.json"     "$DEST/config/$LOWER.example.json"
mv "$DEST/scripts/linux/morfcollector.service"   "$DEST/scripts/linux/$LOWER.service"
